...
export function NoteList({
  ...
  if (isLoading) {
    return (
      <div className="p-4 text-xs font-mono text-ui-dim">Loading notes...</div>
    );
  }

  if (notes.length === 0) {
    return (
      <div className="p-4 text-xs font-mono text-ui-dim text-center">
      ...
      {notes.map((note) => {
        ...
        return (
          <button
            ...
            className={`w-full text-left px-3 py-2 border-b border-obsidian-border/10 transition-colors group ${
              isSelected
                ? 'bg-obsidian-raised text-ui-text'
                : 'text-ui-muted hover:bg-obsidian-surface hover:text-ui-text'
            }`}
          >
            <div className="flex items-center gap-2">
              <FileText
                size={12}
                strokeWidth={1.5}
                className={isSelected ? 'text-phosphor' : ''}
              />
