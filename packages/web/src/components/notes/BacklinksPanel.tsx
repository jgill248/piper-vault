...
export function BacklinksPanel({ noteId, onNavigateToNote }: BacklinksPanelProps) {
  ...
  if (isLoading) {
    return (
      <div className="p-3 text-xs font-label text-on-surface-variant">Loading backlinks...</div>
    );
  }

  if (!backlinks || backlinks.length === 0) {
    return (
      <div className="p-3 text-xs font-label text-on-surface-variant">No backlinks found.</div>
    );
  }
      ...
      <div className="flex flex-col">
        {backlinks.map((entry) => (
          <button
            ...
            className="w-full text-left px-3 py-1.5 text-xs font-label text-secondary hover:text-primary hover:bg-surface transition-colors flex items-center gap-2"
          >
