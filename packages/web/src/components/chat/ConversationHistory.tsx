...
function ConversationItem({ conversation, isActive, onSelect, onDelete }: ConversationItemProps) {
  ...
  return (
    ...
    >
      ...
      {/* Preview snippet */}
      {('messageCount' in conversation) && (conversation as unknown as { messageCount?: number }).messageCount != null && (
        <span className="font-mono text-[8px] text-ui-dim uppercase tracking-wider">
          {String((conversation as unknown as { messageCount: number }).messageCount)} messages
        </span>
      )}
